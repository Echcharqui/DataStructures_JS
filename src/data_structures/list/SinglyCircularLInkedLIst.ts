...
class SinglyCircularLinkedList<T> extends SuperList<T> {
  private _length: number = 0;
  private head: border = {
    linkTo: null
  };

  private next: SinglyNode = null;
  private prev: SinglyNode = null;
  private current: SinglyNode = null;
  ...
  AddFirst (data: T) {
    ...
    if (this.head.linkTo !== null) {
      ...
      for (let i = 0; i < this._length; i++) {
        if (this.current.getIndex === this._length - 1) {
          this.current.setNext = this.head.linkTo
          break
        }
      ...
      NewItem.setIndex = 0
      this.head.linkTo = NewItem
      NewItem.setNext = this.head.linkTo
      this._length += 1
    }
    NewItem = null
    this.destroyer()
  ...
  AddLast (data: T) {
    ...
    if (this.head.linkTo !== null) {
      ...
      newSinglyNode.setNext = this.head.linkTo
      this._length += 1
    } else {
    ...
    for (let i = 0; i < this._length; i++) {
      if (this.current.getData === value) {
        if (this.current.getIndex === 0) {
          this.RemoveFirst()
        } else if (this.current.getIndex === this._length - 1) {
          this.RemoveLast()
        } else {
  ...
  RemoveLast () {
    ...
    if (this.current !== null) {
      for (let i = 0; i < this._length; i++) {
        if (this.current.getIndex === this._length - 1) {
          this.current.setNext = null
          this.prev.setNext = this.head.linkTo
          this._length -= 1
          this.destroyer()
          break
        }
